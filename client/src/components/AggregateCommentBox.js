...
function AggregateCommentBox(props) {
    const { store } = useContext(GlobalStoreContext);
    const { list } = props;

    useEffect(() => {
        console.log("in aggregate comments!")
        ...
        if(event.keyCode == 13){
            ...
            if(event.target.value===""){
              //dont do anything
                ...
                store.addAggregateComment(event.target.value, list);
                //store.addComment(event.target.value);
            }
...
export default AggregateCommentBox;